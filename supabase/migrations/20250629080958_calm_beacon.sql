/*
  ...
  1. New Functions
    - `admin_create_property` - Creates a new property with admin privileges
    - `admin_update_property` - Updates an existing property with admin privileges  
    - `admin_delete_property` - Deletes a property with admin privileges
    - `admin_set_property_amenities` - Sets amenities for a property with admin privileges

  2. Security
    - All functions check for admin authentication using `is_admin_authenticated()`
    - Functions bypass RLS by using SECURITY DEFINER
    - Only authenticated admin users can execute these functions
*/

-- Function to create a property with admin privileges
...
BEGIN
  ...
  INSERT INTO properties (
    ...
    latitud, longitud, imagenes, disponible, destacado
  )
  VALUES (
  ...
  RETURNING 
    ...
    properties.fecha_creacion, properties.fecha_actualizacion;
END;
$$;

-- Function to update a property with admin privileges
...
-- Function to delete a property with admin privileges
...
BEGIN
  ...
  -- Delete the property (cascading will handle amenity relationships)
  DELETE FROM properties WHERE id = property_id;
END;
$$;

-- Function to set property amenities with admin privileges
