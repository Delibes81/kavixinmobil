/*
  ...
  1. Funciones actualizadas
    - `admin_create_property`: Ahora incluye el campo id_interno
    - `admin_update_property`: Ahora incluye el campo id_interno
    - `admin_delete_property`: Función para eliminar propiedades
    - `admin_set_property_amenities`: Función para gestionar amenidades

  2. Seguridad
    - Todas las funciones usan SECURITY DEFINER para bypass RLS
    - Permisos otorgados al rol authenticated

  3. Notas
    - Las funciones manejan el nuevo campo id_interno que fue agregado manualmente
    - Compatible con el formulario de administración existente
*/

-- Create or replace admin_create_property function with id_interno support
...
BEGIN
    INSERT INTO public.properties (
        ...
        NULLIF(property_data->>'id_interno', '') -- Handle empty strings as NULL
    )
...
-- Create or replace admin_update_property function with id_interno support
...
BEGIN
    ...
    SET
        ...
        id_interno = NULLIF(property_data->>'id_interno', ''), -- Handle empty strings as NULL
...
-- Create or replace admin_delete_property function
...
AS $$
BEGIN
    -- Delete property amenities first (foreign key constraint)
...
-- Create or replace admin_set_property_amenities function
...
BEGIN
    ...
    WHERE property_id = admin_set_property_amenities.property_id;
